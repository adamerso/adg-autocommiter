...
# =======================
# Funkcje:
# - Blokada plików >90MB (zatrzymuje auto-commit)
# - Wykrywanie i podsumowanie zmian
# - 10-minutowy timeout przed auto-commit
# - Pull w trakcie timeoutu (bez blokowania)
# - Merge wszystkiego (jeden branch)
...
# =======================
GIT="/usr/bin/git"
LOCKFILE="/tmp/autogit3.lock"

PULL_EVERY=12           # seconds
CHECK_EVERY=12          # seconds (check local changes)
COMMIT_TIMEOUT=600      # 10 minutes in seconds
MAX_FILE_SIZE_MB=99     # warn and add to .gitignore files larger than this
...
# state variables
# =======================
CHANGES_DETECTED_AT=0   # timestamp when changes first detected
PENDING_COMMIT=false    # true if we have pending changes waiting for timeout
...
do_commit_push() {
  ...
  # Check for large files first (auto-adds to .gitignore, doesn't block)
  check_large_files
...
handle_pending_commit() {
  ...
  if [[ "$PENDING_COMMIT" == false ]]; then
    ...
    local commit_time
    commit_time=$(/usr/bin/date -d "@$commit_at" +%H:%M:%S 2>/dev/null || /usr/bin/date -r "$commit_at" +%H:%M:%S 2>/dev/null || echo "za 10 min")
    
    /usr/bin/printf "%b╔══════════════════════════════════════════════════════════════╗%b\n" "$c_magenta" "$c_reset"
    /usr/bin/printf "%b║%b  ⏱️  Auto-commit za 10 minut (o %s)                   %b║%b\n" "$c_magenta" "$c_reset" "$commit_time" "$c_magenta" "$c_reset"
    /usr/bin/printf "%b║%b                                                              %b║%b\n" "$c_magenta" "$c_reset" "$c_magenta" "$c_reset"
    /usr/bin/printf "%b║%b  %b[y]%b = commit NOW z auto-message                            %b║%b\n" "$c_magenta" "$c_reset" "$c_green" "$c_reset" "$c_magenta" "$c_reset"
    /usr/bin/printf "%b║%b  %b[m]%b = wpisz własny message                                 %b║%b\n" "$c_magenta" "$c_reset" "$c_yellow" "$c_reset" "$c_magenta" "$c_reset"
    /usr/bin/printf "%b╚══════════════════════════════════════════════════════════════╝%b\n\n" "$c_magenta" "$c_reset"
...
/usr/bin/printf "\n"
/usr/bin/printf "%b╔══════════════════════════════════════════════════════════════╗%b\n" "$c_green" "$c_reset"
/usr/bin/printf "%b║%b            %b🚀 AUTO-COMMITER 3.0 STARTED%b                       %b║%b\n" "$c_green" "$c_reset" "$c_bold" "$c_reset" "$c_green" "$c_reset"
/usr/bin/printf "%b╠══════════════════════════════════════════════════════════════╣%b\n" "$c_green" "$c_reset"
/usr/bin/printf "%b║%b  Pull co:          %3ds                                      %b║%b\n" "$c_green" "$c_reset" "$PULL_EVERY" "$c_green" "$c_reset"
/usr/bin/printf "%b║%b  Check co:         %3ds                                      %b║%b\n" "$c_green" "$c_reset" "$CHECK_EVERY" "$c_green" "$c_reset"
/usr/bin/printf "%b║%b  Commit timeout:   %3ds (10 min)                             %b║%b\n" "$c_green" "$c_reset" "$COMMIT_TIMEOUT" "$c_green" "$c_reset"
/usr/bin/printf "%b║%b  Max file size:    %3dMB                                     %b║%b\n" "$c_green" "$c_reset" "$MAX_FILE_SIZE_MB" "$c_green" "$c_reset"
/usr/bin/printf "%b╠══════════════════════════════════════════════════════════════╣%b\n" "$c_green" "$c_reset"
/usr/bin/printf "%b║%b  %b[y]%b = commit NOW    %b[m]%b = własny message                    %b║%b\n" "$c_green" "$c_reset" "$c_cyan" "$c_reset" "$c_yellow" "$c_reset" "$c_green" "$c_reset"
/usr/bin/printf "%b╚══════════════════════════════════════════════════════════════╝%b\n\n" "$c_green" "$c_reset"
...
while true; do
  ...
  if (( now - last_pull >= PULL_EVERY )); then
    /usr/bin/printf "%b══════ autocommiter3 cycle (pull) ══════%b\n" "$c_dim" "$c_reset"
