...
repo_hardening_setup() {
  ...
  if [[ -f .gitattributes ]]; then
    if ! grep -qE '^\*\.sh\s+text\s+eol=lf' .gitattributes 2>/dev/null; then
  ...
  if ! grep -qF "${rel} merge=ours" .gitattributes 2>/dev/null; then
    echo "${rel} merge=ours" >> .gitattributes
    changed=true
  fi
  ...
  if [[ "$changed" == true ]]; then
    info "Repo hardening: updating .gitattributes"
...
check_large_files() {
  local large_files
  large_files=$(find . -type f -size +${MAX_FILE_SIZE_MB}M 2>/dev/null | grep -v "^./.git/" || true)
...
check_staged_large() {
  ...
  if [[ "$found_large" == true ]]; then
    for file in "${to_remove[@]}"; do
      local clean="${file#./}"
      grep -qxF "$clean" .gitignore 2>/dev/null || echo "$clean" >> .gitignore
...
printf "%b╠══════════════════════════════════════════════════════════════╣%b\n" "$C_CYAN" "$C_RESET"
printf "%b║%b  🌩️  CLOUD MODE: Auto-sync, no branch splits!                %b║%b\n" "$C_CYAN" "$C_RESET" "$C_CYAN" "$C_RESET"
printf "%b║%b  🔄  Network retry: %dx with backoff                         %b║%b\n" "$C_CYAN" "$C_RESET" "$NETWORK_RETRY_MAX" "$C_CYAN" "$C_RESET"
printf "%b╠══════════════════════════════════════════════════════════════╣%b\n" "$C_RED" "$C_RESET"
