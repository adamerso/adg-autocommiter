#!/bin/bash
# =======================
# ADG AUTO-COMMITER 6.0.0  ♥ I<3U ♥
# =======================
# Major upgrade from v5 with:
# - SSH/HTTPS setup with repo auto-detection
# - Network retry with exponential backoff
# - Git operation timeouts
# - Startup push for pending commits
...
# ═══════════════════════════════════════════════════════════
GIT="/usr/bin/git"
LOCKFILE="/tmp/adg-autocommiter6.lock"
...
show_stats() {
  ...
  /usr/bin/printf "%b[%s %s @ %s]%b 📊 %dm%ds | C:%d M:%d F:%d L:+%d/-%d R:%d\n" \
...
network_retry() {
  ...
  while [[ $max_attempts -eq 0 ]] || [[ $attempt -lt $max_attempts ]]; do
    ...
    if [[ $max_attempts -eq 0 ]]; then
      ...
      attempt_info="attempt $attempt/$max_attempts"
    fi
    
    # Run command with timeout
    if timeout "$GIT_TIMEOUT" "${cmd[@]}" 2>/dev/null; then
    ...
    # Get backoff delay - cycle through array
    local delay_idx=$(( (attempt - 1) % backoff_len ))
    ...
    # Log retry info
    info "🔄 Retrying in ${delay}s... (next: $attempt_info)"
...
setup_git_auth() {
  ...
  /usr/bin/printf "\n"
  /usr/bin/printf "%b╔══════════════════════════════════════════════════════════════╗%b\n" "$C_CYAN" "$C_RESET"
  /usr/bin/printf "%b║%b     🔐 GIT AUTHENTICATION SETUP                              %b║%b\n" "$C_CYAN" "$C_RESET" "$C_CYAN" "$C_RESET"
  /usr/bin/printf "%b╚══════════════════════════════════════════════════════════════╝%b\n" "$C_CYAN" "$C_RESET"
  /usr/bin/printf "\n"
  ...
  if [[ -n "$detected_repo" ]]; then
    ...
    /usr/bin/printf "%b[?]%b Use detected repo? [Y/n]: " "$C_GREEN" "$C_RESET"
  ...
  if [[ -z "$user_repo" ]]; then
    /usr/bin/printf "%b[?]%b Enter GitHub username: " "$C_YELLOW" "$C_RESET"
    local gh_user=""
    if [[ -p /dev/stdin ]] || [[ ! -t 0 ]]; then
    ...
    /usr/bin/printf "%b[?]%b Enter repository name: " "$C_YELLOW" "$C_RESET"
    local gh_repo=""
    if [[ -p /dev/stdin ]] || [[ ! -t 0 ]]; then
...
# ═══════════════════════════════════════════════════════════
# DO PULL WITH OURS STRATEGY (CLOUD EXPERIENCE) + NETWORK RETRY
# ═══════════════════════════════════════════════════════════
do_pull() {
  ...
  if [[ -n "$local_head" ]] && [[ -n "$remote_head" ]] && [[ "$local_head" != "$remote_head" ]]; then
    ...
    if [[ -n "$base" ]] && [[ "$base" != "$local_head" ]] && [[ "$base" != "$remote_head" ]]; then
      ...
      if network_retry "Pull merge" $GIT pull --no-rebase -X ours --no-edit; then
        ok "Pull merge OK"
  ...
  # Normal pull (with retry)
  if network_retry "Pull" $GIT pull --no-rebase -X ours --no-edit; then
    ok "Pull OK"
    _heartbeat
    return 0
  else
    ...
    $GIT merge --abort 2>/dev/null || true
    network_retry "Fetch recovery" $GIT fetch origin 2>/dev/null || true
...
/usr/bin/printf "%b║%b  🔄  Network retry: %dx with backoff                         %b║%b\n" "$C_CYAN" "$C_RESET" "$NETWORK_RETRY_MAX" "$C_CYAN" "$C_RESET"
/usr/bin/printf "%b╠══════════════════════════════════════════════════════════════╣%b\n" "$C_RED" "$C_RESET"
...
# 3. Initial check for large files
check_large_files
